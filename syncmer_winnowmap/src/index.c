...
mm_idx_t *mm_idx_init(int w, int k, int b, int flag,double ds,int s,int pos1,int pos2,int pos3,int pos4)
{
	mm_idx_t *mi;
	if (k*2 < b) b = k * 2;
	if (w < 0) w = 0;
	mi = (mm_idx_t*)calloc(1, sizeof(mm_idx_t));
	mi->w = w, mi->k = k, mi->b = b, mi->flag = flag; mi->ds =ds; mi->s = s;
	mi->pos1 = pos1; mi->pos2= pos2; mi->pos3 = pos3; mi->pos4= pos4;
...
void mm_idx_stat(const mm_idx_t *mi)
{
	...
	fprintf(stderr, "[M::%s] kmer size: %d; window: %d; smer size: %d; downsample: %lf; syncmer indices: %d %d %d %d; is_hpc: %d; #seq: %d\n", __func__, mi->k, mi->w,mi->s,mi->ds,mi->pos1,mi->pos2,mi->pos3,mi->pos4, mi->flag&MM_I_HPC, mi->n_seq);
...
static void *worker_pipeline(void *shared, int step, void *in)
{
    ...
    if (step == 0) { // step 0: read sequences
		...
		for (i = 0; i < s->n_seq; ++i) {
			...
			if (t->l_seq > 0)
				mm_sketch(0, t->seq, t->l_seq, p->mi->w, p->mi->k, t->rid, p->mi->flag&MM_I_HPC, &s->a, p->mi,p->mi->ds,p->mi->s,p->mi->pos1,p->mi->pos2,p->mi->pos3,p->mi->pos4);
...
mm_idx_t *mm_idx_gen(mm_bseq_file_t *fp, int w, int k, int b, int flag, int mini_batch_size, int n_threads, uint64_t batch_size, const char *kmer_freq_filename,double ds,int smer_size,int pos1,int pos2,int pos3,int pos4)
{
	pipeline_t pl;
	...
	pl.fp = fp;
	pl.mi = mm_idx_init(w, k, b, flag,ds,smer_size,pos1,pos2,pos3,pos4);
...
mm_idx_t *mm_idx_build(const char *fn, int w, int k, int flag, int n_threads,double ds,int smer_size,int pos1,int pos2,int pos3,int pos4) // a simpler interface; deprecated
{
	mm_bseq_file_t *fp;
	mm_idx_t *mi;
	fp = mm_bseq_open(fn);
	if (fp == 0) return 0;
	mi = mm_idx_gen(fp, w, k, 14, flag, 1<<18, n_threads, UINT64_MAX, NULL,ds,smer_size,pos1,pos2,pos3,pos4);
	mm_bseq_close(fp);
	return mi;
}

mm_idx_t *mm_idx_str(int w, int k, int is_hpc, int bucket_bits, int n, const char **seq, const char **name,double ds,int smer_size,int pos1,int pos2,int pos3,int pos4)
{
	uint64_t sum_len = 0;
	mm128_v a = {0,0,0};
	mm_idx_t *mi;
	...
	mi = mm_idx_init(w, k, bucket_bits, flag,ds,smer_size,pos1,pos2,pos3,pos4);
	mi->n_seq = n;
	...
	for (i = 0, sum_len = 0; i < n; ++i) {
		...
		if (p->len > 0) {
			a.n = 0;
			mm_sketch(0, s, p->len, w, k, i, is_hpc, &a, mi,ds,smer_size,pos1,pos2,pos3,pos4);
...
mm_idx_t *mm_idx_reader_read(mm_idx_reader_t *r, int n_threads, const char *kmer_freq_filename)
{
	...
	if (r->is_idx) {
		...
		mi = mm_idx_gen(r->fp.seq, r->opt.w, r->opt.k, r->opt.bucket_bits, r->opt.flag, r->opt.mini_batch_size, n_threads, r->opt.batch_size, kmer_freq_filename,r->opt.ds,r->opt.s,r->opt.pos1,r->opt.pos2,r->opt.pos3,r->opt.pos4);
	if (mi) {
